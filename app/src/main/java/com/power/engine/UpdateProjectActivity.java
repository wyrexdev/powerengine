...
public class UpdateProjectActivity extends AppCompatActivity {
	...
	private void initialize(Bundle _savedInstanceState) {
		...
		button1.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View _view) {
				FileUtil.deleteFile(FileUtil.getPackageDataDir(getApplicationContext()).concat("/".concat(getIntent().getStringExtra("name").concat("/"))));
				if (edittext1.getText().toString().equals("")) {
					...
					if (edittext2.getText().toString().equals("")) {
						...
						if (FileUtil.isExistFile(FileUtil.getPackageDataDir(getApplicationContext()).concat("/".concat(edittext2.getText().toString().concat("/"))))) {
							...
							FileUtil.writeFile(FileUtil.getPackageDataDir(getApplicationContext()).concat("/".concat(edittext2.getText().toString().concat("/content/scenes/Game.pew"))), "");
	...
	private void initializeLogic() {
		...
		project = new Gson().fromJson(FileUtil.readFile(FileUtil.getPackageDataDir(getApplicationContext()).concat("/".concat(getIntent().getStringExtra("name").concat("/system/".concat("settings.pre"))))), new TypeToken<HashMap<String, Object>>(){}.getType());
		edittext2.setText(project.get("name").toString());
		edittext1.setText(project.get("package").toString());
		edittext3.setText(project.get("version").toString());
		edittext4.setText(project.get("versionUp").toString());
		imageview1.setImageBitmap(FileUtil.decodeSampleBitmapFromPath(FileUtil.getPackageDataDir(getApplicationContext()).concat("/".concat(getIntent().getStringExtra("name").concat("/system/".concat("icon.jpg")))), 1024, 1024));
		if (project.get("screenType").toString().equals("Landscape")) {
			radiobutton1.setChecked(true);
			radiobutton2.setChecked(false);
		}
		if (project.get("screenType").toString().equals("Portrait")) {
